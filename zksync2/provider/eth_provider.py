...
class EthereumProvider:
    ...
    RECOMMENDED_DEPOSIT_L2_GAS_LIMIT = 721876
    ...
    def get_base_cost(self,
                      ...
                      l2_gas_per_pubdata_byte: int = DEPOSIT_GAS_PER_PUBDATA_LIMIT,
                      l1_max_fee: int = None):
        return self.main_contract.l2_tx_base_cost(l1_max_fee, l2_gas_limit, l2_gas_per_pubdata_byte)
    ...
    def approve_erc20(self,
        ...
        erc20 = ERC20Contract(self._eth_web3.eth,
                              token.l1_address, self._l1_account)
    ...
    def deposit(self,
                ...
                to: HexStr = None,
                operator_tip: int = 0,
                l1_max_priority_fee: int = None,
                l1_max_fee: int = None,
                l1_gas_limit: int = None,
                l2_gas_per_pubdata_byte: int = DEPOSIT_GAS_PER_PUBDATA_LIMIT,
                l2_gas_limit: int = RECOMMENDED_DEPOSIT_L2_GAS_LIMIT
                ):
        if to is None:
        ...
        if l1_max_priority_fee is None:
            l1_max_priority_fee = self._eth_web3.eth.max_priority_fee

        if l1_max_fee is None:
            base_fee_per_gas = self._eth_web3.eth.fee_history(1, 'latest')['baseFeePerGas'][0]
            l1_max_fee = l1_max_priority_fee + base_fee_per_gas

        base_cost = self.get_base_cost(l1_max_fee=l1_max_fee,
                                       l2_gas_per_pubdata_byte=l2_gas_per_pubdata_byte,
                                       l2_gas_limit=l2_gas_limit)

        if token.is_eth():
            # l1_value = amount + base_cost + operator_tip
            # l2_value = amount
            l1_value = amount
            l2_value = amount - base_cost - operator_tip
            return self.request_execute(
                contract_address=to,
                call_data=HexStr("0x"),
                l1_value=l1_value,
                l2_value=l2_value,
                l1_max_priority_fee=l1_max_priority_fee,
                l1_max_fee=l1_max_fee,
    ...
    def request_execute(self,
        ...
        # if l1_max_fee is None:
        #     base_fee_per_gas = self._eth_web3.eth.fee_history(1, 'latest')['baseFeePerGas'][0]
        #     l1_max_fee = l1_max_priority_fee + base_fee_per_gas * 11 // 10
        ...
        tx_hash = self.main_contract.request_l2_transaction(contract_l2=contract_address,
                                                               l2_value=l2_value,
                                                               call_data=call_data,
                                                               l2_gas_limit=l2_gas_limit,
                                                               l2_gas_per_pubdata_byte_limit=l2_gas_per_pubdata_byte,
    ...
    def _get_withdraw_log(self, tx_receipt: TxReceipt, index: int = 0):
        topic = event_signature_to_log_topic(
    ...
    def _finalize_withdrawal_params(self, withdraw_hash, index: int) -> dict:
        tx_receipt = self._zksync_web3.zksync.get_transaction_receipt(
            withdraw_hash)
        log, l1_batch_tx_id = self._get_withdraw_log(tx_receipt, index)
        l2_to_l1_log_index, _ = self._get_withdraw_l2_to_l1_log(
            tx_receipt, index)
