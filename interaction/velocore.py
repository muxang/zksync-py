...
# 读取abi
with open("interaction/abi/velocore.json", "r") as f:
    contract_abi = f.read()

contract_address = "0x46dbd39e26a56778d88507d7aEC6967108C0BD36"
...
def buy(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    while True:
        try:
            amount_out, stable = contract.functions.getAmountOut(amount_in, token_in_address, token_out_address).call()
            break
        except Exception as e:
    ...
    # buy 参数
    buy_params = (amount_out_min, routes, address, deadline)
...
def sell(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    # buy 参数
    sell_params = (amount_in, amount_out_min, routes, address, deadline)
    # Encode arguments
...
def interact_with_velocore(zksync_base_op: ZkSyncBaseOp, address, dapp):
    # buy
    while True:
        try:
