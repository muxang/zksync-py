...
def buy(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    while True:
        try:
            ...
            print(f"获取amountOut错误, 等待10秒后重试: {e}")
            time.sleep(10)
    # 滑点设置
    ...
    try:
        ...
        raise Exception(f"账户 {address} 执行 {dapp.name} 买入交易失败: {e}")
...
def sell(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    try:
        ...
        raise Exception(f"账户 {address} 执行 {dapp.name} 卖出交易失败: {e}")

    return tx_hash

def interact_with_spacefi(zksync_base_op: ZkSyncBaseOp, address, dapp):
    # buy
    while True:
        try:
