...
with open("interaction/abi/mute_router.json", "r") as f:
    contract_abi = f.read()

contract_address = "0x8B791913eB07C32779a16750e3868aA8495F5964"
...
def buy(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    while True:
        try:
            amount_out, stable, fee = contract.functions.getAmountOut(amount_in, token_in_address, token_out_address).call()
            break
        except Exception as e:
    ...
    # buy 参数
    buy_params = (amount_out_min, path, address, deadline, stable)
    # Encode arguments
    call_data = contract_encoder.encode_method("swapExactETHForTokensSupportingFeeOnTransferTokens", args=buy_params)
    try:
...
def sell(zksync_base_op: ZkSyncBaseOp, address, dapp):
    ...
    amount_out_min = amount_out * 999 // 1000
    # buy 参数
    sell_params = (amount_in, amount_out_min, path, address, deadline, stable)
    # Encode arguments
    call_data = contract_encoder.encode_method("swapExactTokensForETHSupportingFeeOnTransferTokens", args=sell_params)
    try:
...
def interact_with_mute(zksync_base_op: ZkSyncBaseOp, address, dapp):
    # buy
    while True:
        try:
