...
def transfer_eth(
    ...
    # print(f"Gas estimation is: {estimate_gas}")
    # print(f"Fee for transaction is: {estimate_gas * gas_price}")
    while True:
        try:
            estimate_gas = zk_web3.zksync.zks_estimate_fee(tx_func_call.tx)
            print(f"ZkSync fee estimation is: {estimate_gas}")
            time.sleep(10)
    ...
    estimate_gas = zk_web3.zksync.zks_estimate_fee(tx_func_call.tx)
    print(f"ZkSync fee estimation is: {estimate_gas}")
    print(f"ZkSync fee estimation is: {estimate_gas.gas_limit}")

    return

    # Convert transaction to EIP-712 format
    tx_712 = tx_func_call.tx712(estimate_gas.gas_limit)
...
if __name__ == "__main__":
    ...
    # provider = HttpWithProxyProvider(endpoint_uri=PROVIDER, proxy_url=proxy)

    # Connect to zkSync network
    zk_web3 = ZkSyncBuilder.build(url=PROVIDER, proxy_url=proxy)
